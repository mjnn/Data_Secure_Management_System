...
**完整表结构（对齐当前前端 Mock + 文档资源模块）**：见 **`docs/DSMS_DATA_MODEL.md`**（含新增 `approval_request`、`submission_task`、`document_resource` 等规划；与上表并存时以数据模型文档为实施清单，并回写本节索引表）。
...
| `dsms-tenant-delete` | 数据安全平台：删除项目 |
| `dsms-submission-tasks-copy-approved` | 数据安全项目：复制已审核填报任务 |
